...
import { AppConfig, ConfigValidationResult, DebugFeature } from "./types";
...
// Enhanced configuration validation using validation module
import {
  performHealthCheck,
...
// Configuration validation function (legacy support)
export const validateConfig = validateConfiguration;

// Enhanced configuration health check
export const performConfigHealthCheck = (): void => {
  try {
...
// Configuration monitor for runtime validation
export const configMonitor = createConfigurationMonitor(config);

// Configuration is ready - validation can be run manually if needed
// Use performConfigHealthCheck() to validate when needed
